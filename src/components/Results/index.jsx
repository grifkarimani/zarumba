                                ...
                                <div className="body">
                                    {sharePlayers.map((player, index) => (
                                        <div key={index} className="player">
                                            ...
                                            <div className={["player-current", player.current < 0 ? "red" : "blue"].join(" ")}>{`Итог по очкам: ${
                                                player.current
                                            }`}</div>
                                            {nominalBallPrice && (
                                                <div
                                                    className={["player-mon", player.current < 0 ? "red" : "blue"].join(" ")}
                                                >{`Реальный итог: ${player.current * ballPrice}`}</div>
                                            )}
                            ...
                            <div className="body">
                                {avers.players.map((player, index) => (
                                    <div key={index} className="player">
                                        ...
                                        {nominalBallPrice && (
                                            <div className={["player-mon", player.current < 0 ? "red" : "blue"].join(" ")}>{`Реальный итог: ${player.current *
                                                ballPrice}`}</div>
                                        )}
                                        <div className="player-totalWhites">{`Белых: ${player.totalWhites}`}</div>
                                        <div className="player-totalYellows">{`Свояков от белого: ${player.totalYellows}`}</div>
                                ...
                                className={[
                                    ...
                                    it.message == "забил белый"
                                        ...
                                        : it.message == "забил желтый"
                                            ? "yellow"
                                            : it.message == "забил красный"
                                                ? "red"
                                                : it.message == "получил штраф"
                                                    ? "bull"
                                                    : it.message == "Реверс начался" || it.message == "Аверс начался" || it.message == "Игра окончена"
                                                        ? "info"
                                                        : ""
                                ].join(" ")}
                            >
